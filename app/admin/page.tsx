...
async function getStats(year: number, quarter: number | null, month: number | null) {
  ...
  const [
    ...
    supabase.from('invoices').select('amount_total').eq('direction','emitida').eq('payment_status','pendiente').is('deleted_at',null).limit(5000),
    supabase.from('invoices').select('amount_total').eq('direction','recibida').eq('payment_status','pendiente').is('deleted_at',null).limit(5000),
