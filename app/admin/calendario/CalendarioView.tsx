              ...
              {monthDays.map((day, i) => {
                ...
                return (
                  <button
                    ...
                    className={`relative px-1 pt-0.5 pb-2.5 text-center font-mono transition-colors hover:bg-emerald-100 ${cls}`}
                    title={tooltip}
                  >
                    {new Date(day.d + 'T00:00:00').getDate()}
                    {flags && (flags.assignment || flags.task || flags.reunion || flags.absence) && (
                      <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 flex gap-0.5">
                        {flags.reunion && <span className="w-2 h-2 rounded-full bg-violet-600" />}
                        {flags.task && <span className="w-2 h-2 rounded-full bg-blue-500" />}
                        {flags.assignment && <span className="w-2 h-2 rounded-full bg-emerald-600" />}
                        {flags.absence && <span className="w-2 h-2 rounded-full bg-pink-500" />}
                      </span>
                    )}
                  </button>
                )
