 ...
 *
 * Lo llama un Vercel Cron cada 30 min (fix #4b) y un widget
 * visual en /admin/sistema (fix #4c).
 */
...
export async function GET() {
  ...
  // 1. Última factura insertada por workflow automático
  const { data: lastDoc, error: e1 } = await supabase
    .from('invoices')
  ...
  const lastTs = lastDoc?.created_at ? new Date(lastDoc.created_at) : null
  const hoursSinceLast = lastTs
    ? (now.getTime() - lastTs.getTime()) / (1000 * 60 * 60)
    : Infinity

  // 2. Conteos en ventanas de tiempo
  const since24h = new Date(now.getTime() - 24 * 60 * 60 * 1000).toISOString()
  const since7d = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString()

  const [{ count: count24h }, { count: count7d }, { count: errors24h }] =
    await Promise.all([
      supabase
        .from('invoices')
