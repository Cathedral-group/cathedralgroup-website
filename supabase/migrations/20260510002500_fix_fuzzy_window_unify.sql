-- Bug fix sesión 10/05 madrugada (auditoría profunda Bug 5):
-- Divergencia entre lógica fuzzy match en n8n (20 días, ±0,5%) y RPC find_fuzzy_duplicate_invoices
-- (7 días default, ±0,05€ absoluto). Resultados no convergen — un duplicado puede pillarse en
-- una capa pero no en la otra.
--
-- Fix: actualizar `forensic_full_check` para invocar `find_fuzzy_duplicate_invoices` con
-- ventana 20 días (alinea con lógica n8n). La tolerancia importe se mantiene en ±0,05€
-- absoluto en la RPC (defensa más estricta que el ±0,5% n8n — bien).
