-- ============================================================================
-- Migration: 20260521050000_documents_registry_add_due_date.sql
-- ----------------------------------------------------------------------------
-- Refactor matview documents_registry: add fecha_vencimiento DATE column.
--
-- Necesario para que el hub `/admin/documentos` pueda filtrar server-side
-- por chips "Vence en 30/60/90 días". Hoy el chip está presente en UI pero
-- la matview NO expone una columna de vencimiento unificada cross-doc-type.
--
-- Mapeo source_table → columna fuente de vencimiento:
--   invoices              → due_date
--   contratos             → fecha_fin
--   seguros               → fecha_vencimiento
--   licencias             → fecha_caducidad
--   certificados          → fecha_caducidad
--   informes              → fecha_vigencia
--   presupuestos          → fecha_validez
--   notas_simples         → fecha_vigencia
--   modelos_fiscales      → NULL (no aplica — gasto puntual)
--   payrolls              → NULL (no aplica — periodicidad mensual)
--   albaranes             → NULL
--   escrituras            → NULL
--   justificantes_pago    → NULL
--   certificaciones_obra  → NULL
--   documentos_otros      → NULL
--
...
-- (CASCADE NO borra cron jobs).
--
-- Reproduce el patrón exacto de la migration anterior
-- (20260521000000_documents_registry_add_project_property.sql):
--  - GRANT SELECT a authenticated/service_role + REVOKE anon
--  - Función refresh con fallback no-CONCURRENT si matview vacía
--  - REFRESH inicial NO-CONCURRENT post-DROP
-- ============================================================================
...
SELECT
  'payrolls', p.id, p.company_id, NULL::UUID, NULL::UUID, NULL::DATE, 'nomina',
...
-- CONTRATOS
SELECT
  'contratos', c.id, c.company_id, c.project_id, c.property_id, c.fecha_fin, 'contrato',
...
SELECT
  'notas_simples', n.id, n.company_id, NULL::UUID, n.property_id, n.fecha_vigencia, 'nota_simple',
  n.fecha_expedicion,
  NULL::NUMERIC,
...
SELECT
  'escrituras', e.id, e.company_id, NULL::UUID, e.property_id, NULL::DATE, 'escritura',
...
-- LICENCIAS
SELECT
  'licencias', l.id, l.company_id, l.project_id, l.property_id, l.fecha_caducidad, 'licencia',
...
SELECT
  'certificaciones_obra', co.id, co.company_id, co.project_id, NULL::UUID, NULL::DATE, 'certificacion_obra',
...
-- CERTIFICADOS
SELECT
  'certificados', ce.id, ce.company_id, ce.project_id, ce.property_id, ce.fecha_caducidad, 'certificado',
  ce.fecha_emision,
  ce.importe,
...
-- INFORMES
SELECT
  'informes', inf.id, inf.company_id, inf.project_id, inf.property_id, inf.fecha_vigencia, 'informe',
  inf.fecha_emision,
...
-- SEGUROS
SELECT
  'seguros', s.id, s.company_id, s.project_id, s.property_id, s.fecha_vencimiento, 'seguro',
  s.fecha_efecto,
  s.prima_total,
...
SELECT
  'modelos_fiscales', mf.id, mf.company_id, NULL::UUID, NULL::UUID, NULL::DATE, 'modelo_fiscal',
...
SELECT
  'justificantes_pago', jp.id, jp.company_id, NULL::UUID, NULL::UUID, NULL::DATE, 'justificante_pago',
  jp.fecha_operacion,
  jp.importe,
...
-- ALBARANES
SELECT
  'albaranes', a.id, a.company_id, a.project_id, a.property_id, NULL::DATE, 'albaran',
  a.fecha_albaran,
  a.subtotal_sin_iva,
...
-- PRESUPUESTOS
SELECT
  'presupuestos', pr.id, pr.company_id, pr.project_id, pr.property_id, pr.fecha_validez, 'presupuesto',
  pr.fecha_emision,
  pr.total,
...
SELECT
  'documentos_otros', do_.id, do_.company_id, do_.project_id, do_.property_id, NULL::DATE, 'otro',
...
-- NUEVO: lookup por fecha_vencimiento (chips "Vence en 30/60/90 días")
CREATE INDEX IF NOT EXISTS idx_documents_registry_vencimiento
  ON public.documents_registry(fecha_vencimiento)
  WHERE fecha_vencimiento IS NOT NULL AND deleted_at IS NULL;
